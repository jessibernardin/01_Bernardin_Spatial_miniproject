...
watershed.state.pitchers <- st_join(pitcher.sf, watershed.state, join=st_is_within_distance, dist=5000)

#get all the data to the raster crs
watershed.state.pitchers <- watershed.state.pitchers %>% 
...
precip.extract <- extract(jan_prate, pitcher.buf.vect, fun = mean, na.rm=TRUE)

#Adding the extracted raster data back to the main dataset
summary.watershed.state.pitchers <- watershed.state.pitchers %>% 
  left_join(., temp.extract) %>% 
  left_join(., precip.extract)


 summary.watershed.state.pitchers %>%
  group_by(watershed) %>%
